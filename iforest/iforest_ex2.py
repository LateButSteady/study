...
outlier_rate = 0.005    # outlier 비율  (기본 : 0.1)
PCA_dim = 3             # 결과 확인할때 쓰는 차원축소 할 차원수

subplot_fig_size = 6   # 큰 figure 안에 subplot

num_train_data      = 2000
num_train_outlier   = 20
num_test_data      = 1000
num_test_outlier   = 100


# 간단한 data 생성
rng = np.random.RandomState(seed)
# Generating training data 
X_train = 0.2 * rng.randn(num_train_data, 2)
...
# Generating outliers
X_outliers = rng.uniform(low=-1, high=5, size=(num_test_data, 2))
...
X_outliers_merge = rng2.uniform(low=-1, high=5, size=(num_train_outlier, 2))
...
X_outliers_merge2 = rng3.uniform(low=-1, high=5, size=(num_test_outlier, 2))
...
X_train_df = X_train_df.assign(y = y_pred_train)
...
# origin data
ax1.scatter(X_train_df.x1, X_train_df.x2, c="white", s=50, edgecolor='k', label="train data")
...
# orgin pred 결과
X_outliers_df = X_outliers_df.assign(y = y_pred_outliers)
ax2.scatter(X_train_df.x1[X_train_df.y == 1], X_train_df.x2[X_train_df.y == 1], 
                c="green", s=50, edgecolor='k', label="train data")
ax2.scatter(X_train_df.x1[X_train_df.y == -1], X_train_df.x2[X_train_df.y == -1], 
                c="red", s=50, edgecolor='k', label="train data")
...
ax3.scatter(X_test_df.x1[X_test_df.y == 1], X_test_df.x2[X_test_df.y == 1], 
                c="green", s=50, edgecolor='k', label="test data")
ax3.scatter(X_test_df.x1[X_test_df.y == -1], X_test_df.x2[X_test_df.y == -1], 
                c="red", s=50, edgecolor='k', label="test data")
